...
static int max_freq_limit_level = 23;
...
static int max_freq_delta = 96000;
...
//used_version_20
static int used_version_20 = 0;
module_param(used_version_20, int, 0644);
MODULE_PARM_DESC(used_version_20, "used_version_20");

static int cpu0_table_length;
static int cpu4_table_length;
static int cpu7_table_length;

int cpufreq_table0[24]={1708800,1708800,1612800,1612800,1497600,1497600,1401600,1401600,1305600,1305600,1209600,1209600,1094400,1094400,998400,998400,902400,902400,806400,806400,691200,691200,595200,595200};

int cpufreq_table4[24]={1996800,1996800,1881600,1881600,1766400,1766400,1670400,1670400,1555200,1555200,1440000,1440000,1324800,1324800,1209600,1209600,1075200,1075200,960000,960000,844800,844800,710400,710400};

int cpufreq_table7[24]={2150400,2150400,2035200,2035200,1900800,1900800,1785600,1785600,1670400,1670400,1555200,1555200,1420800,1420800,1305600,1305600,1190400,1190400,1075200,1075200,960000,960000,844800,844800};

//add for qcom cpufreq
...
static void do_frame_limit_freq(void)
{
	...
	for_each_possible_cpu(i) {
		if(i==7){
			...
			cpu_limits_set_level(i,cpufreq_table0[set_freq_level]);
		}
	}
}
...
int perfmgr_policy_init(void)
{
        ...
        cpu0_table_length= sizeof(cpufreq_table0)/sizeof(cpufreq_table0[0]);
	...
	cpu7_table_length= sizeof(cpufreq_table7)/sizeof(cpufreq_table7[0]);
        ...
        if(cpu0_table_length < cpu4_table_length && cpu0_table_length < cpu7_table_length){
		if(max_freq_limit_level>cpu0_table_length-1)
			max_freq_limit_level=cpu0_table_length-1;
	  }else if(cpu4_table_length < cpu7_table_length){
	...
	pr_debug("%s cpu0_table_length=%d cpu4_table_length=%d cpu7_table_length=%d max_freq_limit_level=%d end\n",
                __func__,cpu0_table_length,cpu4_table_length,cpu7_table_length,max_freq_limit_level);
